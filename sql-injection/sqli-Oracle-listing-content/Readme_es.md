...
[__Enlace al laboratorio__](https://portswigger.net/web-security/sql-injection/examining-the-database/lab-listing-database-contents-oracle)
...
2. __Consultar el las tablas especiales (`ALL_TABLES` y `ALL_TAB_COLUMNS`)__ para descubrir los nombres aleatorios de la tabla de usuarios y sus columnas.
...
- __Búsqueda de Tablas:__ A diferencia de otros motores de base de datos, Oracle utiliza __Vistas de diccionarios de Datos__ para gestionar los metadatos del sistema. Para conocer el nombre de las tablas debemos de consultar la vista __ALL\_TABLES__, la cual contiene una lista de todas las tablas relacionales a las que el usuario actual tiene acceso:
```SQL
...
```SQL
UNION SELECT table_name, NULL FROM ALL_TABLES WHERE table_name LIKE 'users_%'
```
El exploit no usará la clausula `WHERE` en esta consulta, sin embargo usará expresiones regulares para el mismo cometido.

- __Búsqueda de Columnas:__ Una vez obtengamos el nombre de la tabla en la que se encuentran los usuarios, consultaremos la vista __ALL\_TAB\_COLUMNS__, la cual detalla todas las columnas de todas las tablas, vistas y clusters a los que el usuario tiene acceso, con la finalidad de conocer el nombre de las columnas que requerimos:
```SQL
UNION SELECT table_name, column_name FROM ALL_TAB_COLUMNS WHERE table_name = '[table_name]'
```
